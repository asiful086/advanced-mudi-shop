...
import { CREATE_SUBSUBCATEGORY } from "../../../graphql/mutations/subsubcategoryMutation";
import { GET_SUBSUBCATEGORIES } from "../../../graphql/queries/subsubcategoryQuery";

const AdminSubsubcategoryCreatePage = ({ history }) => {
  // 1  ============================= COMPONENT STATE =========================
  ...
  const { onChange, onSubmit, values } = useForm(createSubsubcategoryCallback, {
    photo: "",
    name: "",
    category_id: "",
    subcategory_id: "",
  ...
  const [createSubsubcategory, { loading: mutationLoading }] = useMutation(
    CREATE_SUBSUBCATEGORY,
    {
      update(proxy, { data: { createSubsubcategory: item } }) {
        ...
        if (!state.unmounted) {
          ...
          if (data) {
            proxy.writeQuery({
              query: GET_SUBSUBCATEGORIES,
              data: {
                getSubsubcategories: [item, ...data.getSubsubcategories],
              },
            });
          }
          ...
          setState({
            ...state,
            errors: [],
            successMessage: "Subsubcategory Added Successfully",
          });
        }
      },
      onError(err) {
  ...
  function createSubsubcategoryCallback() {
  ...
  if (firstQueryLoading || secondQueryLoading || mutationLoading) {
...
export default AdminSubsubcategoryCreatePage;
