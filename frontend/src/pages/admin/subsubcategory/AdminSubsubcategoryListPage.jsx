...
import { DELETE_SUBSUBCATEGORY } from "../../../graphql/mutations/subsubcategoryMutation";
const AdminSubsubcategoryListPage = ({ history }) => {
  // 1  ============================= COMPONENT STATE =========================
  ...
  const {
    ...
    data: { getSubsubcategories: mainDatas } = {},
  ...
  const [deleteSubsubcategory, { loading: mutationLoading }] = useMutation(
    DELETE_SUBSUBCATEGORY,
    {
      update(proxy, { data: { deleteSubsubcategory: deletedItem } }) {
        if (!state.unmounted) {
          const data = proxy.readQuery({
            query: GET_SUBSUBCATEGORIES,
          });
          ...
          if (data) {
            proxy.writeQuery({
              query: GET_SUBSUBCATEGORIES,
              data: {
                getSubsubcategories: data.getSubsubcategories.filter(
                  (c) => c.id !== deletedItem.id
                ),
              },
            });
          }
          ...
          setState({
            ...state,
            errors: [],
            successMessage: "Subsubcategory Deleted Successfully",
          });
        }
      },
      onError(err) {
                ...
                {mainDatas &&
                  mainDatas.map((mainData) => (
                    ...
                    >
                      ...
                      >
                        <img
                          src={`/images/subsubcategory/${mainData.photo}`}
                          alt="subsubcategory"
                      ...
                      <td
                        ...
                        data-label="Category"
                      >
                        {mainData.category && mainData.category.name}
                      </td>
                      <td
                        className="text-sm pr-6 whitespace-no-wrap text-gray-800 dark:text-gray-100 tracking-normal leading-4 block sm:table-cell text-right sm:text-center relative py-10 sm:py-0 border-b sm:border-b-0"
                        data-label="Subcategory"
                      >
                        {mainData.subcategory && mainData.subcategory.name}
                      </td>
                      <td
                        className="text-sm pr-6 whitespace-no-wrap text-gray-800 dark:text-gray-100 tracking-normal leading-4 block sm:table-cell
                      ...
                      >
                        ...
                        <Link to={`/admin/subsubcategory/edit/${mainData.id}`}>
...
export default AdminSubsubcategoryListPage;
